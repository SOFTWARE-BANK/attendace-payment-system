...
# ------------------ Service Layer ------------------
class Leave_balancesService:
    """Service layer for Leave_balances operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Leave_balances]:
        """Create a new leave_balances"""
        try:
            obj = Leave_balances(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created leave_balances with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating leave_balances: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Leave_balances]:
        """Get leave_balances by ID"""
        try:
            query = select(Leave_balances).where(Leave_balances.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching leave_balances {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of leave_balancess"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Leave_balances, field):
                        column = getattr(Leave_balances, field)
            ...
            stmt = select(Leave_balances, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Leave_balances, field_name):
                        stmt = stmt.order_by(getattr(Leave_balances, field_name).desc())
                else:
                    if hasattr(Leave_balances, sort):
                        stmt = stmt.order_by(getattr(Leave_balances, sort))
            else:
                stmt = stmt.order_by(Leave_balances.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Leave_balances)
                for condition in conditions:
            ...
            logger.error(f"Error fetching leave_balances list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Leave_balances]:
        """Update leave_balances"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Leave_balances {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated leave_balances {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating leave_balances {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Leave_balances {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted leave_balances {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting leave_balances {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Leave_balances]:
        """Get leave_balances by any field"""
        try:
            if not hasattr(Leave_balances, field_name):
                raise ValueError(f"Field {field_name} does not exist on Leave_balances")
            column = getattr(Leave_balances, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Leave_balances).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching leave_balances by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Leave_balances]:
        """Get list of leave_balancess filtered by field"""
        try:
            if not hasattr(Leave_balances, field_name):
                raise ValueError(f"Field {field_name} does not exist on Leave_balances")
            column = getattr(Leave_balances, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Leave_balances.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching leave_balancess by {field_name}: {str(e)}")
            raise