...
# ------------------ Service Layer ------------------
class Attendance_reasonsService:
    """Service layer for Attendance_reasons operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Attendance_reasons]:
        """Create a new attendance_reasons"""
        try:
            obj = Attendance_reasons(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created attendance_reasons with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating attendance_reasons: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Attendance_reasons]:
        """Get attendance_reasons by ID"""
        try:
            query = select(Attendance_reasons).where(Attendance_reasons.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching attendance_reasons {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of attendance_reasonss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Attendance_reasons, field):
                        column = getattr(Attendance_reasons, field)
            ...
            stmt = select(Attendance_reasons, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Attendance_reasons, field_name):
                        stmt = stmt.order_by(getattr(Attendance_reasons, field_name).desc())
                else:
                    if hasattr(Attendance_reasons, sort):
                        stmt = stmt.order_by(getattr(Attendance_reasons, sort))
            else:
                stmt = stmt.order_by(Attendance_reasons.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Attendance_reasons)
                for condition in conditions:
            ...
            logger.error(f"Error fetching attendance_reasons list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Attendance_reasons]:
        """Update attendance_reasons"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Attendance_reasons {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated attendance_reasons {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating attendance_reasons {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete attendance_reasons"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Attendance_reasons {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted attendance_reasons {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting attendance_reasons {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Attendance_reasons]:
        """Get attendance_reasons by any field"""
        try:
            if not hasattr(Attendance_reasons, field_name):
                raise ValueError(f"Field {field_name} does not exist on Attendance_reasons")
            column = getattr(Attendance_reasons, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Attendance_reasons).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching attendance_reasons by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Attendance_reasons]:
        """Get list of attendance_reasonss filtered by field"""
        try:
            if not hasattr(Attendance_reasons, field_name):
                raise ValueError(f"Field {field_name} does not exist on Attendance_reasons")
            column = getattr(Attendance_reasons, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Attendance_reasons)
                .where(column == field_value)
                .offset(skip)
                .limit(limit)
                .order_by(Attendance_reasons.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching attendance_reasonss by {field_name}: {str(e)}")
            raise