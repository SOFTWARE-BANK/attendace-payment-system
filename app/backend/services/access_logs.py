...
# ------------------ Service Layer ------------------
class Access_logsService:
    """Service layer for Access_logs operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Access_logs]:
        """Create a new access_logs"""
        try:
            obj = Access_logs(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created access_logs with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating access_logs: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Access_logs]:
        """Get access_logs by ID"""
        try:
            query = select(Access_logs).where(Access_logs.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching access_logs {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of access_logss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Access_logs, field):
                        column = getattr(Access_logs, field)
            ...
            stmt = select(Access_logs, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Access_logs, field_name):
                        stmt = stmt.order_by(getattr(Access_logs, field_name).desc())
                else:
                    if hasattr(Access_logs, sort):
                        stmt = stmt.order_by(getattr(Access_logs, sort))
            else:
                stmt = stmt.order_by(Access_logs.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Access_logs)
                for condition in conditions:
            ...
            logger.error(f"Error fetching access_logs list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Access_logs]:
        """Update access_logs"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Access_logs {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated access_logs {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating access_logs {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Access_logs {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted access_logs {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting access_logs {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Access_logs]:
        """Get access_logs by any field"""
        try:
            if not hasattr(Access_logs, field_name):
                raise ValueError(f"Field {field_name} does not exist on Access_logs")
            column = getattr(Access_logs, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Access_logs).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching access_logs by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Access_logs]:
        """Get list of access_logss filtered by field"""
        try:
            if not hasattr(Access_logs, field_name):
                raise ValueError(f"Field {field_name} does not exist on Access_logs")
            column = getattr(Access_logs, field_name)
            ...
            logger.error(f"Error fetching access_logss by {field_name}: {str(e)}")
            raise