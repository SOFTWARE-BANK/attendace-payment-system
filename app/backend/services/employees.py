...
class EmployeesService:
    ...
    async def get_list(
        ...
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Employees, field):
                        column = getattr(Employees, field)
            ...
            stmt = select(Employees, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Employees, field_name):
                        stmt = stmt.order_by(getattr(Employees, field_name).desc())
                else:
                    if hasattr(Employees, sort):
                        stmt = stmt.order_by(getattr(Employees, sort))
            else:
                stmt = stmt.order_by(Employees.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Employees)
                for condition in conditions:
    ...
    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Employees]:
        ...
        try:
            ...
            column = getattr(Employees, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Employees).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
