...
# ------------------ Service Layer ------------------
class Overtime_banksService:
    """Service layer for Overtime_banks operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Overtime_banks]:
        """Create a new overtime_banks"""
        try:
            obj = Overtime_banks(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created overtime_banks with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating overtime_banks: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Overtime_banks]:
        """Get overtime_banks by ID"""
        try:
            query = select(Overtime_banks).where(Overtime_banks.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching overtime_banks {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of overtime_bankss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Overtime_banks, field):
                        column = getattr(Overtime_banks, field)
            ...
            stmt = select(Overtime_banks, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Overtime_banks, field_name):
                        stmt = stmt.order_by(getattr(Overtime_banks, field_name).desc())
                else:
                    if hasattr(Overtime_banks, sort):
                        stmt = stmt.order_by(getattr(Overtime_banks, sort))
            else:
                stmt = stmt.order_by(Overtime_banks.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Overtime_banks)
                for condition in conditions:
            ...
            logger.error(f"Error fetching overtime_banks list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Overtime_banks]:
        """Update overtime_banks"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Overtime_banks {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated overtime_banks {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating overtime_banks {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete overtime_banks"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Overtime_banks {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted overtime_banks {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting overtime_banks {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Overtime_banks]:
        """Get overtime_banks by any field"""
        try:
            if not hasattr(Overtime_banks, field_name):
                raise ValueError(f"Field {field_name} does not exist on Overtime_banks")
            column = getattr(Overtime_banks, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Overtime_banks).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching overtime_banks by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Overtime_banks]:
        """Get list of overtime_bankss filtered by field"""
        try:
            if not hasattr(Overtime_banks, field_name):
                raise ValueError(f"Field {field_name} does not exist on Overtime_banks")
            column = getattr(Overtime_banks, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Overtime_banks.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching overtime_bankss by {field_name}: {str(e)}")
            raise