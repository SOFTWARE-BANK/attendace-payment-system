...
# ------------------ Service Layer ------------------
class Daily_attendancesService:
    """Service layer for Daily_attendances operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Daily_attendances]:
        """Create a new daily_attendances"""
        try:
            obj = Daily_attendances(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created daily_attendances with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating daily_attendances: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Daily_attendances]:
        """Get daily_attendances by ID"""
        try:
            query = select(Daily_attendances).where(Daily_attendances.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching daily_attendances {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of daily_attendancess"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Daily_attendances, field):
                        column = getattr(Daily_attendances, field)
            ...
            stmt = select(Daily_attendances, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Daily_attendances, field_name):
                        stmt = stmt.order_by(getattr(Daily_attendances, field_name).desc())
                else:
                    if hasattr(Daily_attendances, sort):
                        stmt = stmt.order_by(getattr(Daily_attendances, sort))
            else:
                stmt = stmt.order_by(Daily_attendances.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Daily_attendances)
                for condition in conditions:
            ...
            logger.error(f"Error fetching daily_attendances list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Daily_attendances]:
        """Update daily_attendances"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Daily_attendances {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated daily_attendances {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating daily_attendances {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete daily_attendances"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Daily_attendances {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted daily_attendances {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting daily_attendances {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Daily_attendances]:
        """Get daily_attendances by any field"""
        try:
            if not hasattr(Daily_attendances, field_name):
                raise ValueError(f"Field {field_name} does not exist on Daily_attendances")
            column = getattr(Daily_attendances, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Daily_attendances).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching daily_attendances by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Daily_attendances]:
        """Get list of daily_attendancess filtered by field"""
        try:
            if not hasattr(Daily_attendances, field_name):
                raise ValueError(f"Field {field_name} does not exist on Daily_attendances")
            column = getattr(Daily_attendances, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Daily_attendances.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching daily_attendancess by {field_name}: {str(e)}")
            raise