...
class ApprovalsService:
    """Service layer for Approvals operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Approvals]:
        ...
        try:
            ...
            logger.info(f"Created approvals with id: {obj.id}")
            return obj
    ...
    async def get_by_id(self, obj_id: int) -> Optional[Approvals]:
        """Get approvals by ID"""
        try:
            query = select(Approvals).where(Approvals.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching approvals {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of approvalss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Approvals, field):
                        column = getattr(Approvals, field)
            ...
            stmt = select(Approvals, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Approvals, field_name):
                        stmt = stmt.order_by(getattr(Approvals, field_name).desc())
                else:
                    if hasattr(Approvals, sort):
                        stmt = stmt.order_by(getattr(Approvals, sort))
            else:
                stmt = stmt.order_by(Approvals.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Approvals)
                for condition in conditions:
            ...
            logger.error(f"Error fetching approvals list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Approvals]:
        """Update approvals"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Approvals {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated approvals {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating approvals {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Approvals {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted approvals {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting approvals {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Approvals]:
        """Get approvals by any field"""
        try:
            if not hasattr(Approvals, field_name):
                raise ValueError(f"Field {field_name} does not exist on Approvals")
            column = getattr(Approvals, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Approvals).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching approvals by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Approvals]:
        """Get list of approvalss filtered by field"""
        try:
            if not hasattr(Approvals, field_name):
                raise ValueError(f"Field {field_name} does not exist on Approvals")
            column = getattr(Approvals, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Approvals.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching approvalss by {field_name}: {str(e)}")
            raise