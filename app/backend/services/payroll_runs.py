...
class Payroll_runsService:
    ...
    async def get_list(
        ...
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Payroll_runs, field):
                        column = getattr(Payroll_runs, field)
            ...
            stmt = select(Payroll_runs, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Payroll_runs, field_name):
                        stmt = stmt.order_by(getattr(Payroll_runs, field_name).desc())
                else:
                    if hasattr(Payroll_runs, sort):
                        stmt = stmt.order_by(getattr(Payroll_runs, sort))
            else:
                stmt = stmt.order_by(Payroll_runs.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Payroll_runs)
                for condition in conditions:
    ...
    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Payroll_runs]:
        ...
        try:
            ...
            column = getattr(Payroll_runs, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Payroll_runs).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
