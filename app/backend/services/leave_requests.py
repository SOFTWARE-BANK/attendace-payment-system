...
# ------------------ Service Layer ------------------
class Leave_requestsService:
    """Service layer for Leave_requests operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Leave_requests]:
        """Create a new leave_requests"""
        try:
            obj = Leave_requests(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created leave_requests with id: {obj.id}")
            return obj
    ...
    async def get_by_id(self, obj_id: int) -> Optional[Leave_requests]:
        """Get leave_requests by ID"""
        try:
            query = select(Leave_requests).where(Leave_requests.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching leave_requests {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of leave_requestss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Leave_requests, field):
                        column = getattr(Leave_requests, field)
            ...
            stmt = select(Leave_requests, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Leave_requests, field_name):
                        stmt = stmt.order_by(getattr(Leave_requests, field_name).desc())
                else:
                    if hasattr(Leave_requests, sort):
                        stmt = stmt.order_by(getattr(Leave_requests, sort))
            else:
                stmt = stmt.order_by(Leave_requests.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Leave_requests)
                for condition in conditions:
            ...
            logger.error(f"Error fetching leave_requests list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Leave_requests]:
        """Update leave_requests"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Leave_requests {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated leave_requests {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating leave_requests {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Leave_requests {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted leave_requests {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting leave_requests {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Leave_requests]:
        """Get leave_requests by any field"""
        try:
            if not hasattr(Leave_requests, field_name):
                raise ValueError(f"Field {field_name} does not exist on Leave_requests")
            column = getattr(Leave_requests, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Leave_requests).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching leave_requests by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Leave_requests]:
        """Get list of leave_requestss filtered by field"""
        try:
            if not hasattr(Leave_requests, field_name):
                raise ValueError(f"Field {field_name} does not exist on Leave_requests")
            column = getattr(Leave_requests, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Leave_requests.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching leave_requestss by {field_name}: {str(e)}")
            raise