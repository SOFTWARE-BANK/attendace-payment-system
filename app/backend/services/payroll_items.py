...
# ------------------ Service Layer ------------------
class Payroll_itemsService:
    """Service layer for Payroll_items operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Payroll_items]:
        """Create a new payroll_items"""
        try:
            obj = Payroll_items(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created payroll_items with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating payroll_items: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Payroll_items]:
        """Get payroll_items by ID"""
        try:
            query = select(Payroll_items).where(Payroll_items.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching payroll_items {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of payroll_itemss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Payroll_items, field):
                        column = getattr(Payroll_items, field)
            ...
            stmt = select(Payroll_items, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Payroll_items, field_name):
                        stmt = stmt.order_by(getattr(Payroll_items, field_name).desc())
                else:
                    if hasattr(Payroll_items, sort):
                        stmt = stmt.order_by(getattr(Payroll_items, sort))
            else:
                stmt = stmt.order_by(Payroll_items.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Payroll_items)
                for condition in conditions:
            ...
            logger.error(f"Error fetching payroll_items list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Payroll_items]:
        """Update payroll_items"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Payroll_items {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated payroll_items {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating payroll_items {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete payroll_items"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Payroll_items {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted payroll_items {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting payroll_items {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Payroll_items]:
        """Get payroll_items by any field"""
        try:
            if not hasattr(Payroll_items, field_name):
                raise ValueError(f"Field {field_name} does not exist on Payroll_items")
            column = getattr(Payroll_items, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Payroll_items).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching payroll_items by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Payroll_items]:
        """Get list of payroll_itemss filtered by field"""
        try:
            if not hasattr(Payroll_items, field_name):
                raise ValueError(f"Field {field_name} does not exist on Payroll_items")
            column = getattr(Payroll_items, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Payroll_items.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching payroll_itemss by {field_name}: {str(e)}")
            raise